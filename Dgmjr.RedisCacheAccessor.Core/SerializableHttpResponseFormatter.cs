...
{
    private static readonly string[] _supportedMediaTypes = new[] { TextMediaTypeNames.Html, ApplicationMediaTypeNames.Json, TextMediaTypeNames.Plain, ApplicationMediaTypeNames.Xml,
                ApplicationMediaTypeNames.OctetStream, MultipartMediaTypeNames.FormData,
                ApplicationMediaTypeNames.FormUrlEncoded,
                VideoMediaTypeNames.Mp4, VideoMediaTypeNames.Mpeg, VideoMediaTypeNames.Ogg,
    ...
    public override async Task WriteResponseBodyAsync(OutputFormatterWriteContext context)
    {
        ...
        if (response == null)
        {
            await context.HttpContext.Response.BodyWriter.AsStream().WriteAsync(new ReadOnlyMemory<byte>(response.Content), default);
        }
    }
}
