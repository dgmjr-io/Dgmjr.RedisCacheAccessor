...
public class SerializableHttpResponse
{
    ...
    {
        get =>
            Headers.TryGetValue(HReqH.ContentType, out var contentType)
                ? contentType
    ...
    {
        get => ContentType.ToMediaType();
        set => ContentType = value.Name;
    }
    ...
    {
        get => Content.ToUTF8String();
        set => Content = value.ToUTF8Bytes();
    }
    ...
    {
        get => _headers;
        set => _headers = new(value, StringComparer.OrdinalIgnoreCase);
    }
    ...
    public DateTimeOffset CreatedDateTime
    {
        get =>
            DateTimeOffset.Parse(
                Headers.TryGetValue(XCreatedDateTime, out var createdDateTime)
                    ? createdDateTime
                    : UnixEpoch
            );
        set => Headers[XCreatedDateTime] = value.ToString();
    }
    public TimeSpan OriginalExpiration
    {
        get =>
            TimeSpan.FromSeconds(
                double.Parse(
                    Headers.TryGetValue(XOriginalExpiration, out var originalExpiration)
                        ? originalExpiration
                        : "0"
                )
            );
        set => Headers[XOriginalExpiration] = value.TotalSeconds.ToString();
    }
    public DateTimeOffset ExpirationDateTime
    {
        get => CreatedDateTime + OriginalExpiration;
        set => OriginalExpiration = value - CreatedDateTime;
    }
    public TimeSpan TimeToLive
    {
        get =>
            TimeSpan.FromTicks(
                Math.Abs((DateTimeOffset.Now - (CreatedDateTime + OriginalExpiration)).Ticks)
            );
        set { }
    }

    public SerializableHttpResponse(HttpResponseMessage message, TimeSpan? expiration = default)
        : this(expiration)
    {
        ...
        ContentType =
            ...
            ?? (
                message.Headers.TryGetValues(HttpResponseHeaderNames.ContentType, out var values)
                    ? Join(", ", values)
                    : message.Content.Headers.TryGetValues(
        ...
        _headers = _headers
            .Concat(message.Headers.ToDictionary(x => x.Key, x => x.Value.First()))
    ...
    public static SerializableHttpResponse BadRequest() =>
        new(default(TimeSpan?))
        {
            ...
            Headers = new() { [HttpResponseHeaderNames.ContentType] = TextMediaTypeNames.Plain, }
        };
