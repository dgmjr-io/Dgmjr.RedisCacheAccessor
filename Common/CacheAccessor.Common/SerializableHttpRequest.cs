...
public class SerializableHttpRequest
{
    ...
    {
        get => BytesContent?.ToUTF8String();
        set => BytesContent = value?.ToUTF8Bytes();
    }
    private StringDictionary _headers = new(StringComparer.OrdinalIgnoreCase);
    public StringDictionary Headers
    {
        get => _headers ??= new StringDictionary(StringComparer.OrdinalIgnoreCase);
        set =>
            _headers = new StringDictionary(
    ...
    public SerializableHttpRequest(HttpRequestMessage message)
    {
        ...
        Method = HttpRequestMethod.TryParse(message.Method.Method, out var method)
            ? method.Value
    ...
    public static implicit operator HttpRequestMessage(SerializableHttpRequest request)
    {
        var message = new HttpRequestMessage(
            new HttpMethod(((HttpRequestMethod)request.Method).Name),
            request.RequestUri
        )
        {
            Content = request.ContentType.ToMediaType().IsText()
                ? new StringContent(request.StringContent)
    ...
    public static implicit operator SerializableHttpRequest(HttpRequestMessage message) =>
        new(message);
}
